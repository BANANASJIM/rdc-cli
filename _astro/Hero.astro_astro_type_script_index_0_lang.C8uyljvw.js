...
ibuffer
descriptors
pixel`},{cmd:"rdc cat /draws/11/pipeline/topology",output:`eid:      11
...
=	vkCmdBeginRenderPass(C=Clear, D=Clear)	1	1	0	12	12	0	0	0	0`}]],o=document.getElementById("replay-lines"),p=document.getElementById("replay-terminal"),i=20,u=30,m=1400,E=2e3;let l=0;function r(e){return new Promise(t=>setTimeout(t,e))}function a(){p.scrollTop=p.scrollHeight}function f(){const e=document.createElement("div");e.className="flex items-start",e.innerHTML='<span class="text-gpu-green mr-2 select-none font-bold shrink-0">$</span>';const t=document.createElement("span");t.className="text-gray-200",e.appendChild(t);const n=document.createElement("span");return n.className="replay-cursor",n.textContent="█",e.appendChild(n),o.appendChild(e),{el:e,span:t}}async function g(e){const{el:t,span:n}=f();a();for(const s of e)n.textContent+=s,a(),await r(i);t.querySelector(".replay-cursor")?.remove()}function h(e){const t=document.createElement("div");if(t.className="text-gray-400 pl-0 whitespace-pre",/^\t*[A-Z_]+(\t[A-Z_]+)+$/.test(e))return t.className="text-gray-500 pl-0 whitespace-pre",t.textContent=e.replace(/\t/g,"  "),t;const n=e.match(/^(\w[\w\s]*?)\s{2,}(.+)$/);if(n){const s=document.createElement("span");s.className="text-gray-500",s.textContent=n[1],t.appendChild(s),t.appendChild(document.createTextNode("  "));const c=document.createElement("span");return c.className="text-gray-300",c.textContent=n[2],t.appendChild(c),t}return e.includes("	")?(t.textContent=e.replace(/\t/g,"  "),t.className="text-gray-300 pl-0 whitespace-pre",t):/^\s*[{}\[\]"]/.test(e)?(t.className="text-gray-400 pl-0 whitespace-pre",t.textContent=e,t):(t.textContent=e,t)}async function _(e){if(e)for(const t of e.split(`
`))o.appendChild(h(t)),a(),await r(u)}async function T(){for(;;){const e=d[l%d.length];l++,o.innerHTML="";for(const t of e)await g(t.cmd),await r(200),await _(t.output),await r(m);await r(E)}}T();
